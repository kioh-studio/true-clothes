SCREEN_SPEC:
  id: home.main
  route: "/home"
  design_path: "design/screen/home/main/"
  ...
  screen:
    system_ui:
      - "Reserve top inset so logotype clears status bar and notch / camera cutout (platform safe-area padding)."
    safe_areas:
      - "Horizontal padding 24 logical px for scroll content; tabs use wider side inset (40) per layout rhythm."
  ...
  layout:
    ...
    body:
      ...
      tabs_layout:
        ...
        touch:
          min_height_logical: 48
          horizontal_padding_each_tab: 4
      active_content: "Show the list/cards for the selected tab."
    footer:
      type: "none_for_mvp"

  implementation_note:
    gradient_alpha_top_stop: "Spec: #4D5051 @ 35% opacity at stop 0. Legacy Compose build used ~50% alpha; prefer the spec for new cross-platform work unless brand review says otherwise."
  ...
  responsive_layout:
    - "Keep header and tabs visible; body scrolls (lazy list / sliver list on large datasets)."
    - "Use scaled dimensions; see design/responsive-ui.md."
    - "Avoid horizontal scroll; composition card scales down uniformly on narrow widths."
    - "Verify ~320 logical px width: tabs do not collide—allow wrap only if spec later changes (default single row with space-between)."

  ux_skill_reference:
    note: "See `.agents/skills/ui-ux-advicer/SKILL.md` in this repo for general UI/UX checklists (accessibility, touch targets, motion)."
    accessibility:
      - "Logotype and tabs on gradient: verify contrast for white/thin text on upper gradient stop."
      - "Tab row: expose selected state to assistive technologies (e.g. selected semantics / accessibility label)."
      - "Tappable outfit card: accessibility label should describe the action (open outfit), not only 'image'."
    touch_and_interaction:
      - "Each tab: minimum ~48 logical px hit height; expand width with horizontal padding on label."
      - "Provide pressed/active feedback on tab and card (platform default or custom)."
    performance:
      - "Use lazy lists when outfit count can grow; stable keys per outfit id."
    typography:
      - "Thin tab labels at 14sp: if contrast fails on devices, bump weight slightly or increase inactive opacity floor to 0.75."
    motion:
      - "Tab switch: content swap without aggressive animation; respect reduced-motion user preference."
