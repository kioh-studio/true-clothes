...
// extraction from raw RGBA pixels. Pure TypeScript PORT of the Deno original
// (supabase/functions/generate-outfits/engine/colorCluster.ts) — same
// algorithm, same thresholds; keep the two in sync manually (client can't
// import a Deno-only file, the server can't import an RN module — same
// duplication convention as TONE12_AVOID). No native/Expo imports here so it
// is importable from Jest without a device; the uri → pixels wrapper lives in
// colorClusterUri.ts.
//
