...
import { GARMENT_MEASUREMENTS } from '../../data/measurements';
import {
  ItemCategory, ColorProfile, PrimaryColor, ColorLightness, ColorSaturation,
  GraphicsProfile, FabricProfile, FitItem, Pattern, ItemFit, FabricName,
...
// [primaryColor, lightness, saturation, hue (0-360 or undefined), sat%, lum%, undertone]
...
const COLOR_MAP: Record<string, ColorEntry> = {
  //                primary        light      sat         hue   s%   l%   undertone
...
const SEASON_BY_WEIGHT: Record<FabricProfile['fabricWeight'], Season> = {
  light: 'summer', medium: 'allSeason', heavy: 'winter',
};
type Season = FabricProfile['season'];
...
const fabricProfileOf = (material: string | undefined, category: ItemCategory): FabricProfile => {
  const def: Partial<FabricDefaults> = material ? (FABRIC_DEFAULTS[material] ?? {}) : {};
...
// Color-aware style tag adjustments: dark colors shift toward formal/minimalist,
// bold/vivid colors shift toward streetwear/y2k, earth tones toward oldmoney/bohemian.
...
// Material-aware style tag adjustments
...
export function styleTagsOf(type: string, color?: string, material?: string): string[] {
  ...
  // Collect boost tags from color and material
  const boosts = new Set<string>();
  if (color && COLOR_STYLE_BOOSTS[color]) {
    COLOR_STYLE_BOOSTS[color].forEach(t => boosts.add(t));
  }
  if (material && MATERIAL_STYLE_BOOSTS[material]) {
    MATERIAL_STYLE_BOOSTS[material].forEach(t => boosts.add(t));
  }

  // Reorder: tags that appear in both base AND boosts come first (strongest signal).
  // Then remaining base tags. Then boost-only tags that weren't in base.
...
// ─── Pattern inference from item name/type ───────────────────────────────────
...
function deriveFit(item: ClothingItem): ItemFit {
  // Prefer explicit fit from the item
  if (item.fit) {
    const normalized = FIT_FROM_STRING[item.fit.toLowerCase()];
    if (normalized) return normalized;
  }
  // Check item name for fit keywords
...
function deriveWarmth(material: string | undefined, category: ItemCategory): number {
  if (material && MATERIAL_WARMTH[material] !== undefined) return MATERIAL_WARMTH[material];
  return CATEGORY_WARMTH[category] ?? 2;
}

// ─── Formality derivation (1–5) ─────────────────────────────────────────────
...
const COLOR_FORMALITY_SHIFT: Partial<Record<PrimaryColor, number>> = {
  black: 0.5, charcoal: 0.5, navy: 0.5, // darker = slightly more formal
  white: 0, cream: 0, beige: 0,
  red: -0.5, yellow: -0.5, orange: -0.5, pink: -0.5, // brighter = more casual
};
...
function deriveFormality(type: string, color: PrimaryColor, material?: string): number {
  ...
  // Material shift
  if (material === 'Wool' || material === 'Cashmere' || material === 'Silk') base += 0.5;
  if (material === 'Nylon' || material === 'Fleece') base -= 0.5;
  if (material === 'Denim') base -= 0.3;
  return Math.max(1, Math.min(5, base));
}

// ─── Statement strength (0–5) ───────────────────────────────────────────────
// How "loud" this item is visually. High = hero piece, low = base/support.
...
function deriveFabricName(material: string | undefined): FabricName | undefined {
  ...
  return FABRIC_NAME_MAP[material];
}

// ─── Build FitItem from ClothingItem ─────────────────────────────────────────

export const toFitItem = (item: ClothingItem): FitItem => {
  ...
  return {
    ...
    garmentMeasurements: GARMENT_MEASUREMENTS[item.id],
