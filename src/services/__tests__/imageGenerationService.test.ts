...
// The server extraction schema (generate-item-image/prompt.ts) returns
// can_layer/print_scale/drape/visual_interest, but the client's RawMetadata/
// toDomain never declared or mapped them, so they were silently dropped at
// this exact boundary before useAddWizard ever saw them. This suite verifies
// extractItemsWithImages() (the public entry point wrapping toDomain) now
// carries them through, mirroring the existing primary_hex/secondary_hex/
// distressed threading.
...
describe('extractItemsWithImages — visual enrichment + can_layer threading', () => {
  it('carries print_scale/drape/visual_interest/can_layer from the raw response into GarmentMetadata', async () => {
